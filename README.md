...
- 현재 상태: **Phase 0~4 완료 — 파일에서 솔버 덱까지 인장 한 줄기가 닫혔다.**
  다음은 Phase 5(물성 확장 — DMA 처리부터).
- **운영 서버는 v0.1.4 에 머물러 있다.** Phase 2~4 가 아직 배포되지 않았고,
  올릴 때 마이그레이션이 16건 한꺼번에 돈다 — 백업을 먼저 받는다(아래 배포 절차).
