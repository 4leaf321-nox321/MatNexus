...
try {
    ...
    throw "$AppPath 를 옮길 수 없습니다. 실행 중인 앱과 그 폴더(하위 폴더 포함)에 들어가 있는 창을 닫고 다시 시도하세요. (서비스는 다시 띄웠습니다)"
}
...
} elseif ($installedServices.Count -gt 0) {
    Write-Host '서비스가 등록돼 있지만 롤백 전에 멈춰 있었습니다 — 띄우려면 .\service.ps1 -Action Start'
} else {
    Write-Host '시작:'
