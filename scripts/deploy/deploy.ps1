...
# --- 코드의 씨앗 맞추기 -----------------------------------------------------------
#
# 기본 축·물성 항목·시험 종류·형식 프로파일은 코드에 씨앗이 있는데, 설치 때 한 번
# 심고 그 뒤로는 아무도 다시 안 심었다. 코드가 채널·규칙·선택지를 더해도 기존 DB 는
# 못 받았다 — 실측(2026-09-12): DMA 마스터커브 자동 등록 규칙이 08-31 에 코드에
# 들어갔는데 그 전에 설치된 서버는 오류 없이 그 기능이 없었다. 이제 배포마다 맞춘다.
# **관리자가 고친 값은 안 건드린다** — 없는 것만 만들고 빠진 것만 채운다.
if ($SkipMigrations) {
    Write-Log '씨앗 맞추기 건너뜀 (마이그레이션과 함께)'
} else {
    Push-Location (Join-Path $AppPath 'backend')
    try {
        Invoke-Native '씨앗 맞추기 실패' { & $backendPython scripts\refresh_builtins.py }
    } catch {
...
if ($installedServices.Count -gt 0) {
    ...
    Write-Host ''
    Write-Host "배포한 것이 $AppPath 에, 직전 버전이 $prevPath 에 있습니다."
    exit 0
}
Write-Host '시작 — 창 두 개:'
...
Write-Host ''
Write-Host "재부팅해도 저절로 뜨게 하려면(한 번):  .\service.ps1 -AppPath '$AppPath' -Action Install   (관리자 PowerShell)"
Write-Host ''
