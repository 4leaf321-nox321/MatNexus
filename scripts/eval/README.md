...
## 하루 한 번 저절로 — `nightly.ps1`

세션마다 값이 들어 한 번의 통과 수로는 「나아졌다」 를 말할 수 없다. 다섯 번을 손으로 돌리는
동안 매번 25분씩 기다리고 json 을 열어 견줬다(2026-09-20). 그래서 야간 작업이 쌓는다.

```powershell
cd scripts\eval
# 토큰: 화면 → 내 계정 → 토큰 에서 기준선용 계정으로 발급해 .pat 에 한 줄 (gitignore 됨)
.\nightly.ps1 -Register        # 매일 03:00, 현재 사용자로 (claude CLI 로그인이 그 사용자 것이라)
.\nightly.ps1 -Status          # 마지막 실행과 trend.csv 끝 5줄
.\nightly.ps1                  # 지금 한 번
```

- 개발 백엔드(8011)가 안 떠 있으면 **띄우지 않고** 로그 한 줄로 그만둔다.
- **전부 돌린 실행만** `results/trend.csv` 에 한 줄 쌓인다 — `--ids`·`--only` 로 낱개를 돌린
  것은 안 들어간다(「1/1」 이 「29/34」 뒤에 서면 추세가 아니다).
- 직전 줄보다 통과가 3 이상 떨어지면 `results/nightly.log` 에 「꺾임」 을 남긴다. 알림 채널이
  아직 없어 그것이 전부다.

사람이 보는 파일은 `trend.csv` 하나다. 미달의 **까닭**은 `latest.md` 의 줄에서 본다.
