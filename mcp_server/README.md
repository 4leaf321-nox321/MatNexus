...
- **재료·시료 API 의 `density` 는 SI 가 아니다** — 화면 표시 단위(tonne/mm3)로
  ...
  (2026-09-24). 서버가 곁에 `density_si`(kg/m³)를 두게 되어 **MCP 는 그것을 낸다**
  (`_density_si`). **카드 API(`POST /fitting/cards`)의 `density` 도 SI(kg/m³)다** —
