...
- **재료·시료 API 의 `density` 는 한동안 SI 가 아니었다** — 화면 표시 단위(tonne/mm3)로
  ...
  선언 물성·물려받는 값도 `si_unit` 을 함께 싣는다 — 값만 넘기던 자리였다. **시편 치수도
  같은 날 밤 SI(m)** 가 됐다(`list_specimens` · `get_sample` 이 그대로 싣는다) — 전에는 넣은
  단위(대개 mm)를 되돌려 줬다. `create_specimen` 은 전부터 `length_unit: "mm"` 을 적어 보낸다.
