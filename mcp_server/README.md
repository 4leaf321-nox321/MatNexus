...
### 문헌 카탈로그에 직접 넣기 — **출처 없는 값은 안 받는다**
    ...
    ★ migrate_catalog_property(...)    폐기된 키의 값·매핑·별칭을 후속 키로 (미리보기 기본, 관리자)
...
### 통계 — 흩어짐
    ...
    get_distribution(material, test_type, 정규·로그정규·와이블을 나란히 — best 를
                     orientation, scalar_key?) 그대로 옮기지 마라, delta_aicc 를 봐라
    spec_gap(material?, limit)        선언한 값 vs 잰 값 — 차이가 큰 것이 위로
    spread_by_group(scalars?, group_by) 재료군·분류별 상자그림 — 이상치가 재시험 후보
...
**도구 전부를 부르려면 둘을 더 준다** — 없으면 그 도구가 「부르지 못한 도구」 에 남는다.

```powershell
$env:MATNEXUS_PROBE_FIT_MATERIAL = '<인장 MD 채택 시험이 여럿인 재료 id>'   # create_card_from_tests
...
`build_deck` 이 고른 문헌 재료에 덱에 실을 값이 모자란다는, 자료에 따른 거절이다(건너뛴
이유가 함께 실려 온다).
...
- **재료·시료 API 의 `density` 는 한동안 SI 가 아니었다** — 화면 표시 단위(tonne/mm3)로
  오고 `density_unit` 이 함께 왔다. 필드 이름에 단위를 박았다가 알루미늄이
  `2.68e-09 kg/m3` 로 나갔다. 그 뒤 값과 단위를 함께 실었는데, 그래도 **한 응답에서
  밀도만 표시 단위이고 선언 물성은 SI** 라 해석 연동 쪽이 「전부 SI」 로 읽을 뻔했다
  (2026-09-24). 서버가 곁에 `density_si` 를 뒀다가, 같은 날 저녁 **`density` 자체를 SI 로
  바꾸고 `density_si` 를 걷었다**(ADR 0036 D4). `_density_si` 는 둘 다 읽는다 — `density_si`
  가 있는 v1.257 서버도, 없는 뒤의 서버도. **카드 API(`POST /fitting/cards`)의 `density` 도 SI(kg/m³)다** —
  화면 숫자를 그대로 옮기면 서버가 범위 밖(0.5~50,000)이라고 거절한다(2026-09-11).
  선언 물성·물려받는 값도 `si_unit` 을 함께 싣는다 — 값만 넘기던 자리였다. REST 의 **시편
  치수도 같은 날 밤 SI(m)** 가 됐다(전에는 넣은 단위, 대개 mm) — MCP 는 **바뀐 것이 없다**:
  시편은 처음부터 `sizes`(SI)를 `sizes_si` 로 단위와 함께 옮겼고, `create_specimen` 은 전부터
  `length_unit: "mm"` 을 적어 보낸다(09-25 프로브로 확인. 앞 커밋에 「그대로 싣는다」 고
  적었던 것은 틀린 말이었다).
- **늘 지켜야 하는 규칙은 서버 안내문(`instructions`)에 둔다**(2026-09-24). 「단위를 안
  말하면 mm·N·tonne 으로 답한다」 를 안내(GUIDE)의 `units` 절에만 두었더니, 그 절을 안 부른
  점검 세션이 SI 로만 답했다(`trust` 절만 읽었다). GUIDE 는 AI 가 **골라 읽는** 것이고, 모든
  대화에 실리는 것은 `MCPServer(instructions=...)` 한 문장뿐이다. 안내문은 서버를 켤 때
  읽으므로 고치면 서버를 다시 띄운다(GUIDE 는 부를 때마다 읽는다).
- **감사에는 사람과 AI 가 함께 남는다.** 서버가 `X-Client: mcp` 를 붙이고,
  ...
  **쓰기 도구는 전부 흔적을 남겨야 한다** — 09-25 에 도구마다 경로를 맞대 보니 재료·시료·
  시편 등록, 문헌 카탈로그 쓰기, 측정 의뢰 작성이 아무것도 안 남기고 있었다(AI 가 문헌 값을
  지어 넣어도 사람이 넣은 값과 구별이 안 됐다). 이제 `backend/tests/architecture/
  test_mcp_writes_audited.py` 가 `_send` 호출마다 백엔드 라우트를 찾아 감사를 부르는지 본다 —
  **새 쓰기 도구를 더하면 거기서 멈춘다.** 미리보기처럼 쓰지 않는 호출은 그 파일의 `READS` 에
  사유와 함께 적는다.
