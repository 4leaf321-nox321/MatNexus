...
- **자료를 고칠 권한은 `permissions.require_edit` 하나가 정한다**(ADR 0035). 등록자 ·
  편집을 받은 부서 · 자료 관리자 · 시스템 관리자. 자료·정의·장비·워크벤치 작업이 전부
  이 판정이다. 모듈에서 부서나 등록자를 직접 견주지 않는다 — 개편 전에는 모듈마다
  판정이 다섯 갈래라 같은 재료에서 필드는 고쳐지는데 문헌 연결은 막혔다. **부서
  관리자라는 자리를 고칠 권한의 근거로 쓰지 않는다** — 그 역할은 멤버 · 커넥터와
  수신함 · 의뢰 알림뿐이다. 쓰기 라우트를 새로 만들면
  `tests/architecture/test_edit_rule_routes.py` 가 멈춰 세운다 — 판정을 부르거나 사유를
  적는다.
...
- **표 칸에 글자 크기를 적지 않는다.** 크기는 `<Table>` 에 한 번 준다. 흐리게
  (muted)는 **값이 아닌 것**에만 — 빈 목록 안내·「없음」·아이콘. 칸마다 정하면
  한 줄 안에서 크기가 세 가지로 갈리고, 어느 열이 회색인지에 규칙이 없어진다
  (2026-09-10 지적: 재료 목록에서 번호·이름이 작고 회색인데 두께·시료는 크고
  검정이었다). `tests/architecture/test_frontend_table_tone.py` 가 검사한다.
- **시험은 데이터로 그려진 것을 기다린다.** API 가 불렸는지(`toHaveBeenCalled`)나
  데이터보다 먼저 서는 틀·빈 칸·`<option>` 을 기다린 뒤 곧바로 동기(`getBy…`)로 보면,
  느린 머신에서 아직 안 그려진 화면을 본다. 실측(2026-09-24): v1.257.0 CI 에서 감사 화면
  시험이 행위 이름을 드롭다운 옵션에서 먼저 찾아 졌고, 흉내 낸 응답을 30ms 늦춰 전체를
  돌리니 17건이 더 드러났다. 「없다」 를 보는 시험은 빈 화면에서 **틀린 이유로 통과**한다.
  의심되면 응답을 늦춰 돌려 본다.
