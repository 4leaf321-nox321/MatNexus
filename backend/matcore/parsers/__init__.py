...
class ParsedTest:
    ...
    **같은 함수**를 타야 한다(`normalize_conditions`)."""
    ...
    """파일이 **재료에 대해** 적어 온 값들. 재료 칸 이름 → 원문.

    ## 업로드는 이것을 안 쓴다

    다른 DB 에서 통째로 넘어온 파일에는 재료의 두께·밀도·푸아송비까지 들어 있다.
    그런데 **시험 하나가 재료를 고치면 안 된다** — 재료 아래 시험이 100건이면
    같은 칸이 100번 덮어써지고, 그중 하나만 옛 값이어도 카드와 덱이 조용히
    바뀐다. 시편 치수는 시험 하나가 시편 하나를 보므로 「빈 칸만 채운다」로
    막을 수 있었지만, 재료는 그렇게 못 막는다.

    **이관은 다른 이야기다.** 한 번 하는 일이고, 그때는 파일이 원천이다. 그래서
    `identity` 와 같은 자리에 둔다 — 프로파일이 선언하고, 미리보기가 보여 주고,
    실제로 쓰는 것은 이관 경로뿐이다."""
    material_units: dict[str, str] = field(default_factory=dict)
    """그 값이 **무슨 단위로 적혀 있었나.** 재료 칸 이름 → 단위 기호.

    JSON 에는 단위 줄이 아예 없다(`readers/json_tables.py`). 프로파일이 선언하지
    않으면 읽는 쪽이 SI 로 보는데, 두께를 mm 로 적어 온 파일에서 그것은 1000배다."""

    sample: dict[str, str] = field(default_factory=dict)
    """파일이 **시료에 대해** 적어 온 값들. 시료 칸 이름 → 원문.

    제조사·유통사·생산일 같은 것들이다. 쓰이는 자리는 `material` 과 같다."""
