...
UNITS: dict[str, Unit] = {
    ...
    for unit in (
        ...
        # **뉴턴 계열의 기준 한 줄**(2026-09-20). 물리적으로 Pa 와 같은데, 조합(ADR 0031)이
        # 접두어를 풀려면 서명이 같은 줄이 표에 있어야 한다 — 없으면 `kN/mm2`(= 1 GPa,
        # 규격서가 탄성계수를 이렇게 적는다)·`N/cm2`·`MN/m2` 가 전부 「모르는 단위」 다.
        # 이 한 줄이 그 계열 전부를 연다. `N/mm2` 는 전부터 별칭(→ MPa)으로 됐다.
        _u("N/m2", "stress", "1"),
