...
class BlockSpec:
    ...
    **행이 `si_unit` 을 들고 있으면 그것이 이긴다** — 경화식 파라미터는 식마다
    ...
    kind_priority: int | None = 50
    """이 블록이 들어 있으면 카드의 **종류**가 되는가, 여럿이면 누가 먼저인가.

    한 재료에 경화식·점탄성·재료 기본 정보 카드가 섞여 쌓이면 이름만으로는 무엇이
    무엇인지 안 보인다 — 카드가 든 블록 가운데 가장 특징적인 것이 종류다. 탄성 블록은
    거의 모든 카드에 들어 있어 종류가 못 된다(`None`). **작을수록 먼저.** 화면이 이
    값으로 종류를 고르므로 확장이 블록을 더하면 그 이름이 저절로 종류가 된다 —
    전에는 화면이 블록 이름 여섯을 박아 두고 있었다(2026-09-12). 기본 50 은 내장
    블록들 뒤다."""
    ...
    from_tests: tuple[str, ...] = ()
    """이 블록을 **내는 시험 종류의 키**(`tensile` · `dma_sweep`). 덱 준비도가 「이 블록이
    없다 → 이 시험을 하면 생긴다」 를 답하는 근거다(2026-09-16). 비어 있으면 시험이
    아니라 사람이 적거나(선언) 문헌에서 채택하는 블록이다 — 열물성이 그렇다."""
