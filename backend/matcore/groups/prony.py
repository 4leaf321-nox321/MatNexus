...
@register(
    ...
    params=(
        ParamSpec(
            ...
            choice_help={
                # **명사형으로 적는다.** 화면의 모달 설명과 같은 자리에 뜬다 —
                # 한쪽만 「~합니다」 면 말투가 갈린다(2026-09-05).
                "pooled": (
                    "시편들의 점을 모두 모아 한 번에 적합. 시편 사이의 차이가 잔차로 "
                    "남음 — 그 값이 크면 「이 재료를 계수 한 벌로 표현하기 어렵다」 는 "
                    "뜻. 평균은 그 차이를 지움."
                ),
                "averaged": (
                    "시편마다 적합한 뒤 계수를 평균. 시편별 값을 따로 볼 때 사용. "
                    "**완화시간은 공통 격자에 고정** — 자유롭게 맞추면 시편마다 다른 "
                    "값으로 수렴해 평균이 성립하지 않음."
                ),
                "representative": (
                    "시편 하나의 계수를 그대로 사용. 시편이 한둘이거나 하나가 확실히 "
                    "좋을 때. 나머지 시편의 정보는 미반영."
                ),
            },
            help="여러 시편의 마스터커브에서 계수 한 벌을 구하는 방법.",
        ),
        ParamSpec(
            name="terms",
            label="항 수",
            ...
            default=0,
            help=(
                "Prony 항의 수. 0 이면 BIC 로 자동 선택 — 항이 많을수록 잘 맞지만 "
                "과적합, 그 균형의 기준. 시편별 평균은 공통 격자가 필요해 0 이면 4 항."
            ),
        ),
        ParamSpec(
            name="representative",
            ...
            type="str",
            default="",
            help="비우면 잔차가 가장 작은 시편. 「대표 하나 고르기」 에만 적용.",
        ),
    ),
    makes_values=(
        Produced(
