...
FRONTEND_ONLY = {
    "profile",
    # **복합 물성 영역 — 화면 자리가 먼저다**(2026-09-05, ADR 0026 제안). 사이드바를
    # 갈라 관계자와 개발 방향을 논의하는 단계라 개요 화면 하나만 있고, 백엔드는
    # MX 의 현행 시험 데이터 체계를 확인한 뒤 `stacks`·`methods`·`inverse` 로
    # 갈라져 들어온다. 그때 이 프론트 모듈도 그 이름들로 나뉘고 예외는 사라진다.
    "composite",
}
...
BACKEND_ONLY = {
    # `search` 도 여기 있었다. **v1.206.0 에 화면이 생겨 예외가 없어졌다** —
    # 「화면 없는 하부 기능」 이라고 적어 뒀는데, 정작 사람이 무엇을 찾는지 아는
    # 채로만 쓸 수 있는 검색이 화면마다 흩어져 있었다.
    ...
    # 온톨로지는 **AI 가 쓰는 지도**다(ADR 0028). 사람은 재료 상세에서 시료를
    # 누르고 시험으로 가면 되지만 AI 에게는 링크가 없다 — 그 자리를 메우는
    # 세 엔드포인트뿐이고, 화면을 따로 두면 「어디서 보나」 가 둘이 된다.
    "ontology": "AI 용 지도 — 사람 화면은 각 도메인 화면이 이미 그 노릇을 한다",
}
