"""기준정보 값 이름이 바뀔 때 따라와야 하는 일.

## 왜 훅인가

기준정보 값의 표기를 고치면 그 값을 쓰는 쪽에서 할 일이 생길 수 있다. Grade 가
그렇다 — Grade 는 재료 이름을 만들므로(ADR 0004) 값이 바뀌면 재료·시료·시편·
시험 이름을 전부 다시 계산해야 한다.

그 일을 **기준정보 모듈이 직접 하면 안 된다.** 기준정보가 재료 서비스를 부르는 순간
모듈이 서로를 부르게 되고(`tests/architecture` 가 막는다), 축이 늘어날 때마다
기준정보 모듈이 새 도메인을 알아야 한다.

그래서 반대로 한다 — **쓰는 쪽이 자기 뒤처리를 등록한다.** 기준정보는 "이 축의
...
#: 자기가 읽는다. 넘기기 시작하면 기준정보의 모양이 훅 계약에 스며든다.
