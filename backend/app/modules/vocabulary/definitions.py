...
BUILTIN_VOCABULARIES: list[tuple[str, str, str, int, str | None, str | None]] = [
    ...
    # **장비 유형이 칸을 정의한다.** UTM 에 「승온 속도」 칸이 있으면 안 되고 DSC 에
    ...
    # **시험실.** 자유 문자열로 두면 「2공장 3층」·「2공장3층」·「제2공장 3F」 가
    # 갈리고, 그러면 "그 장비 어디 있나" 에 답이 셋 나온다.
    ("lab", "시험실", "open", 52, None, None),
    ...
    # **사업부 아래의 조직.** 「생기연」·「재료연구팀」 처럼 장비를 실제로 들고 있는
    # 단위다. 부모가 사업부라 **장비는 조직만 가리키면 되고 사업부는 부모를 타고
    # 올라가면 나온다** — 둘 다 적게 하면 같은 답을 두 번 저장하고 언젠가 갈린다.
    #
    # **외부 기관도 여기 산다.** 위탁 시험 기관은 사업부 부모가 없는 값으로 들어오고,
    # 사내/외부는 `equipment_units.ownership` 이 가른다. 축을 나누면 같은 이름이
    # 두 목록에 따로 쌓이고 합칠 방법이 없다(거래처 축이 유통사와 주 벤더를 한
    # 축에 둔 것과 같은 판단).
    ("org", "조직", "open", 46, "division", None),
