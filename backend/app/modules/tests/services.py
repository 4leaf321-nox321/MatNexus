...
# --- 조건 값 ----------------------------------------------------------------
#
# `app/shared/conditions.py` 로 옮겼다 — 측정 의뢰의 항목이 같은 조건 칸을 같은
# 규칙으로 받는다. 여기 이름은 그대로 둔다(`services.normalize_conditions` 로 부르는
# 자리가 여럿이다).

condition_fields = conditions.condition_fields
normalize_conditions = conditions.normalize_conditions
_to_si_checked = conditions._to_si_checked
_as_number = conditions._as_number
...
def _apply_dimensions(db: Session, run: TestRun, parsed: ParsedTest) -> None:
    ...
    전에는 사람이 「장비 치수 반영」 을 눌러야 시편에 들어갔고, 그것도 **빈
    ...
    시편 쪽 「반영」 은 그대로 둔다. 시험이 없는 시편도 치수를 가져야 하고,
...
def _store_curves(db: Session, run: TestRun, parsed: ParsedTest, *, version: str) -> None:
    ...
    **한 파일이 곡선을 여럿 낼 수 있다.** 실측: TA DMA850 주파수-온도 스윕은
    ...
    다시 파싱하면 이전 것을 **전부** 지우고 새로 쓴다. 다시 읽기은 새 사실이 아니라
