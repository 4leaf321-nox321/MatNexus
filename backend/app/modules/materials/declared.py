...
절반이다 — 단위를 그 차원으로 검사하므로 **「비열 자리에 열전도율」가 막힌다.**
...
#: **로트를 증명하는 문서.** 이 출처의 값만 시료 층으로 강제된다 — 아래
#: `check` 의 층 규칙이 이것으로 가른다.
LOT_SOURCES = frozenset({"millsheet"})
...
def check(
    ...
    그래서 시료 층 항목을 재료에 적는 것은 **출처가 가른다**: `millsheet`
    (로트를 증명하는 성적서)만 시료로 안내하며 막고, 나머지는 공칭값으로
    받는다. 반대 방향(재료 층 항목을 시료에)은 그대로 막는다 — 탄성계수는
    로트마다 다르지 않다. 카드·밀시트 대조는 밀도와 같은 규칙이다: 시료 값이
    먼저, 없으면 재료 공칭.

    **`datasheet` 를 둘로 쪼갠 이유**(2026-09-06): 한 칸이 「밀시트·데이터시트」
    로 성격이 정반대인 두 문서를 담고 있었다 — 밀시트는 로트 증명이고 제품
    데이터시트는 Grade 스펙이다. 안전을 위해 둘 다 막았더니 벤더 제품시트에서
    온 공칭 강도값(문헌 카탈로그 1,197건)이 통째로 잠겼다.
    ...
    for row in rows:
        ...
        if spec is None:
            ...
            if (
                ...
                and str(row.get("source") or "") not in LOT_SOURCES
            ):
                ...
                hint = (
                    " 제품 데이터시트의 공칭값이면 출처를 「제품 데이터시트」 로 적으세요 "
                    "— 「밀시트」 는 그 로트의 값이라 시료에 붙습니다."
