...
class ItemOut(BaseModel):
    ...
    facts: dict[str, int] = {}
    """**판정에 쓸 사실.** 화면이 「이 단계는 끝났나」 를 이 숫자로 정한다 —
    시험이면 마스터커브·적합·채택 수, 카드면 확정 여부.

    세는 곳을 서버에 둔 이유: 워크벤치가 도메인 API 를 직접 부르면 남의 도메인을
    알게 되고, 그 방향은 되돌리기 어렵다(ADR 0024 의 경계)."""

    material_id: uuid.UUID | None = None
    """이 대상이 딸린 재료. **주소가 아니라 id 다** — 화면이 「그 재료로 가기」 를
    그린다. 글로벌 피팅은 재료 화면에 있는데(ADR 0020) 바구니에는 시험이 담기므로,
