...
class Settings(BaseSettings):
    ...
    """내보낸 데이터가 쌓이는 폴더. 안 적으면 `filestore` 옆의 `exports`.

    **파일스토어 안에 두지 않는다.** 그 폴더는 오펀 정리가 훑는 자리라, 우리가
    일부러 만든 폴더를 「DB 에 행이 없는 것」 으로 보고 지울 수 있다.

    운영은 백업과 같은 드라이브(D:)를 쓰는 편이 낫다 — 곡선까지 넣으면 크고,
    그 파일을 사람이 기계에서 가져간다."""
    ...
    **영원히 안 잡힌다**(오펀은 정의상 DB 에 없는 것이다). 실측으로 확인했다 —
    ...
    signup_email_domains: list[str] = Field(default_factory=lambda: ["samsung.com"])
    """**가입 신청**을 받는 메일 도메인. 아이디가 이 중 하나로 끝나야 신청이 된다
    (2026-09-16 요청 — 회사 메일이 아닌 아이디가 쌓이는 것을 막는다).

    빈 목록이면 제한하지 않는다. 시스템 관리자가 직접 만드는 계정(`POST /accounts`)
    은 이 제한을 받지 않는다 — `admin` 처럼 도메인 없는 아이디는 그 길로만 생긴다.
    """
