...
(전단탄성계수·선팽창계수(CTE)·비열·열전도율)은 인장시험이 안 주므로 화면의 「못 견준
...
RANGES: dict[str, dict[str, tuple[float, float, str]]] = {
    "Metal": {
        ...
        "선팽창계수(CTE)": (11e-6, 13e-6, "1/K"),
        # **표가 아는 기호를 쓴다**(`matcore.units`) — 가운뎃점이 아니라 온점이다.
        # 가운뎃점을 넣었다가 재료 목록이 통째로 500 이 났다(2026-08-29).
        "비열": (450.0, 500.0, "J/(kg.K)"),
        "열전도율": (45.0, 60.0, "W/(m.K)"),
    },
    "Plastic": {
        "탄성계수": (1.8e9, 3.2e9, "GPa"),
        "선팽창계수(CTE)": (60e-6, 90e-6, "1/K"),
