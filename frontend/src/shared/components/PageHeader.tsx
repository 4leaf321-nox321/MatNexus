  ...
  return (
    <div
      className={cn(
        ...
        //
        // 스크롤 상자는 `AppShell` 의 `<main>`(`overflow-auto p-6`)이고, **그
        // 안쪽 여백 24px 이 이 세 값을 전부 정한다.**
        //
        //   `-top-6`  sticky 는 스크롤 상자의 **내용 상자** 기준으로 붙는다. `top-0`
        //             이면 여백만큼 내려가 붙고, 그 위 24px 띠로 **스크롤되는 본문이
        //             그대로 비쳐 보인다**(2026-08-30 실측). 24px 끌어올려 상자의
        //             보이는 맨 위에 붙인다.
        //   `pt-6`    그렇게 끌어올린 만큼을 제 안쪽 여백으로 되받는다 — 안 그러면
        //             붙은 순간 제목이 화면 맨 끝에 닿는다.
        //   `-mx-6`   가로도 덮어야 한다. 없으면 내용이 머리글 **옆으로** 샌다.
        //             `-mt-6` 은 붙기 전 자리를 원래대로 두려고 함께 쓴다.
        sticky &&
          'bg-background sticky -top-6 z-20 -mx-6 -mt-6 mb-4 border-b px-6 pt-6 pb-3'
      )}
    >
      {back && (
        <Link
