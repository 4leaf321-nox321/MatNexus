 ...
 *
 * 둘 다 **왼쪽에 목록, 오른쪽에 내용**으로 갈리고 그 안에서 또 갈린다. 그 3단을
 * 1600 에 밀어 넣으면 맨 오른쪽(곡선·시편 표)이 눌린다.
 *
 *     /test-runs/:id   시험 조건 | 탭(요약값 | 곡선)
 *     /materials/:id   재료 목록 | 탭(시료 목록 | 시편 표)
 *
 * **표만 그리는 화면은 넣지 않는다** — 재료 목록(`/materials`)이 그렇다. 4K 에서
 * 한 줄이 화면을 가로지르면 눈이 행을 놓친다. 여기 있는 둘은 표가 화면의 일부일
 * 뿐이고, 그 표도 옆에 붙은 열이 폭을 잡아 준다.
 */
export const WIDE = [/^\/test-runs\/[^/]+$/, /^\/materials\/[^/]+$/]
