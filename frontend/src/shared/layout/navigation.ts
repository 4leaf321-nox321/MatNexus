...
export const NAV_GROUPS: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      // 있던 것은 화면뿐이었다. 고치는 자리는 '관리' 의 「온톨로지 편집」 이다.
      ...
      //
      // **메뉴 이름은 「온톨로지 조회」 다**(2026-09-25, 사용자 요청 — 전에는 「기준정보」).
      // 코드·API·화면 안의 개념 이름은 그대로 `vocabulary`·기준정보다.
      { label: '온톨로지 조회', icon: Tags, to: '/vocabulary' },
  ...
  },
  {
    ...
    items: [
      ...
      { label: '부서 정보', icon: Building2, to: '/admin/workspaces', audience: 'system_admin' },
      {
        ...
        // 같은 이름이면 관리자는 어느 쪽이 진짜인지 모른다 — 전에 '통계'·
        // '내보내기' 를 stub 으로 두었다가 같은 일이 났다. 「조회」 와 「편집」 으로
        // 가른다(2026-09-25, 전에는 「기준정보」 · 「기준정보 편집」).
        label: '온톨로지 편집',
        icon: Tags,
        to: '/admin/vocabulary',
        audience: 'system_admin',
      ...
      {
        // **저장소 정리와 휴지통을 여기 아래로 넣었다.** 셋은 같은 질문의 세 쪽이다 —
        // 「드라이브에 남은 것」(서버)·「우리가 쌓은 것」(정리)·「지운 것이 어디로
        // 갔는지」(휴지통). 메뉴에 따로 서 있으면 디스크가 찰 때 한쪽만 보고 「치울 게
        // 없다」 고 결론 낸다. 화면 안에서 탭으로 갈리고 주소는 그대로다(휴지통은
        // 2026-09-14 에 탭으로 옮겼다 — 지운 것을 볼 자리가 없던 이관 사고(08-28)의 답은
        // 그대로 `/admin/trash` 다).
        label: '서버',
        icon: Server,
        to: '/server',
