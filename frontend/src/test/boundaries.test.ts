...
const ALLOWED: Record<string, { to: string; why: string }[]> = {
  ...
  vocabulary: [
    {
      to: 'catalog',
      why: '물성 항목 축에 「물성 매핑」 표가 붙는다 — 문헌 키 · 사내 항목 · 잰 값이 한 줄에 서는 표. 매핑(`property_links`)은 문헌 카탈로그의 것이라 표를 그리는 것은 catalog 의 일이고, 기준정보 화면은 그 축의 자리만 내준다(바로 아래 units 와 같은 판단). 반대로 뒤집으면 카탈로그가 기준정보 화면을 알게 된다.',
    },
    {
      to: 'units',
  ...
  formulas: [
    {
      to: 'tests',
      why: '계산식은 저장하기 전에 **실제 채택 결과 하나로 돌려 본다**(ADR 0030 D6). 문법이 맞는데 뜻이 틀린 식은 그것으로만 드러난다. 「채택된 시험」 목록은 시험 모듈의 것이라 그 API 를 그대로 부른다 — 여기서 목록을 다시 만들면 채택 여부·가시 범위를 두 벌로 판단하게 된다.',
    },
  ],
  tests: [
    {
  ...
  fitting: [
    {
      to: 'materials',
      why: '글로벌 피팅(결과 목록·메모·삭제·다시 맞추기)은 재료 화면의 패널(GroupsPanel)이고, CAE 카드 탭의 「점탄성 카드·속도 의존 카드」 버튼은 그 패널을 모달로 그대로 세운다(2026-09-05). 두 벌로 만들면 결과 목록과 카드 만들기가 갈리고, 카드는 그 결과를 근거(group_result_id)로 가리키므로 같은 화면이어야 한다.',
    },
    {
      to: 'statistics',
  ...
  materials: [
    {
      to: 'catalog',
      why: '재료 상세의 물성 탭이 문헌 연결 섹션(LinkedCatalogSection)을 끼워 넣는다 — 이 재료가 문헌(카탈로그)의 어느 등급인지 잇고, 문헌 값으로 빈 선언 물성을 채우는 자리다(ADR 0027). 연결·채택은 카탈로그의 일이라 그 모듈이 제공하고 재료 화면은 조립만 한다. 반대로 뒤집으면(catalog 이 재료 상세를 앎) 카탈로그가 사내 화면을 알게 된다.',
    },
    {
      to: 'vocabulary',
