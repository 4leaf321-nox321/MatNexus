...
const ALLOWED: Record<string, { to: string; why: string }[]> = {
  tests: [
    ...
    {
      to: 'vocabulary',
      why: '업로드 창이 장비를 어휘로 받는다(ADR 0010). 장비 이름은 도메인 값이지 공통 부품이 아니라 shared 로 못 올리고, 어휘 쪽은 화면을 몰라야 해서 반대로도 못 뒤집는다. 재료 모듈이 같은 이유로 어휘를 부른다.',
    },
    {
      to: 'processing',
  ...
  materials: [
    {
      to: 'vocabulary',
      why: '재료·시료의 속성이 어휘를 가리킨다(ADR 0010). 제조사·강종은 도메인 값이지 공통 부품이 아니라 shared 로 올릴 수 없고, 어휘 쪽은 화면을 몰라야 해서 반대로도 못 뒤집는다. 재료 폼이 어휘 피커를 조립하는 방향이 맞다.',
    },
    {
      to: 'statistics',
