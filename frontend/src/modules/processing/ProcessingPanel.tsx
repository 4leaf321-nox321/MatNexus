...
const SOURCE_HELP: Record<string, string> = {
  ...
  declared:
    '재료에 사람이 적어 둔 값입니다(문헌·규격). 잰 값이 아니므로, 곡선이 성겨 탄성계수를 못 잴 때 같은 자리에 씁니다.',
}
...
export function ProcessingPanel({
  ...
  return (
    <section className="mb-8">
      ...
      {/* **4 대 6 이다. 고정폭이 아니다.**
          전에는 왼쪽이 `minmax(0,26rem)` 이었는데, `minmax` 의 아래끝이 0 이라
          **26rem 은 상한일 뿐 하한이 아니었다** — 변수 목록을 펴거나 창이 좁으면
      ...
      {/* **화면 오른쪽 끝에 붙는다.** 이 안(본문 폭 제한)에 두면 본문과
