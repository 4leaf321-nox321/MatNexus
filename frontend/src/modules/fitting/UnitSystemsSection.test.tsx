...
describe('덱 단위계', () => {
  it('붙박이는 지우기가 없고, 만든 것만 지운다', async () => {
    ...
    expect(screen.queryByLabelText('SI (kg · m · s · Pa) 삭제')).toBeNull()
    await userEvent.click(screen.getByLabelText('LS-DYNA (GPa) 삭제'))
  ...
  it('관리자가 아니면 만들기·지우기가 없다', async () => {
    ...
    await screen.findByText('LS-DYNA (GPa)')
    expect(screen.queryByRole('button', { name: '단위계 생성' })).toBeNull()
    expect(screen.queryByLabelText('LS-DYNA (GPa) 삭제')).toBeNull()
  })

  it('셋을 고르면 유도된 기호가 보이고, 만들기는 그 셋을 보낸다', async () => {
    render(<UnitSystemsSection canEdit />)
    await screen.findByText('LS-DYNA (GPa)')
    await userEvent.click(screen.getByRole('button', { name: '단위계 생성' }))
