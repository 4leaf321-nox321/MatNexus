...
export const KINDS: {
...
}[] = [
  {
    key: 'family',
    label: '곡선에 맞추는 식',
    purpose: '곡선에 식을 맞춰 계수를 구한다 — 경화식·점도식처럼 카드에 담기는 식.',
    where: '재료 상세 › CAE 카드 › 탄소성(유변…) 카드의 식 후보',
    inputs: 'x 는 곡선의 열(진소성변형률·전단율…). 계수(K, n …)는 식이 데이터에서 구한다.',
    example: 'K * pow(e0 + x, n)  — Swift 경화식',
  },
  {
    key: 'scalar_step',
    label: '값에서 값을 내는 식',
    purpose: '앞 단계가 낸 값들로 새 값 하나를 낸다.',
    where: '처리 레시피의 단계 목록 — 레시피에 넣으면 결과에 값이 남는다',
    inputs: '처리 단계가 내는 값(항복강도·인장강도·탄성계수…). 목록에서 고른다.',
    example: 'proof_stress / tensile_strength  — 항복비',
  },
  {
    key: 'column_step',
    label: '곡선에서 열을 내는 식',
    purpose: '곡선의 열들로 새 열 하나를 더한다.',
    where: '처리 레시피의 단계 목록 — 결과 곡선에 열이 하나 는다',
    inputs: '원본 파일의 채널(변위·하중…)과 앞 단계가 만든 열(진응력…). 목록에서 고른다.',
