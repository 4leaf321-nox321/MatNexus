...
vi.mock("@/modules/pipelines/api", async () => {
  const actual = await vi.importActual<
    typeof import("@/modules/pipelines/api")
  >("@/modules/pipelines/api");
  return {
    ...actual,
    pipelinesApi: {
...
describe("수집함", () => {
  it("시편 필요가 기본이고, 후보가 여럿이면 고르라고 한다", async () => {
    ...
    expect(inbox).toHaveBeenCalledWith(
      expect.objectContaining({ status: "needs_specimen" }),
    );
  });
  ...
  it("항목을 열면 후보를 보여 주고, 고른 것만 붙인다", async () => {
    ...
    expect(
      await screen.findByText("SECC_MDOI_1.0__01__MD_02"),
    ).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(assign).toHaveBeenCalledWith("i1", { specimen_id: "s2" }),
    );
  });
  ...
  it("이유 없이는 못 버린다", async () => {
    ...
    await waitFor(() =>
      expect(discard).toHaveBeenCalledWith("i1", "시험 실패"),
    );
  });
  ...
  it("다른 시편을 찾아 붙일 수 있다", async () => {
    ...
    await waitFor(() =>
      expect(assign).toHaveBeenCalledWith("i1", { specimen_id: "s9" }),
    );
  });
});
...
describe("실패 탭", () => {
  it("다시 읽기를 준다", async () => {
    ...
    expect(
      await screen.findByText("읽을 방법이 없습니다."),
    ).toBeInTheDocument();
